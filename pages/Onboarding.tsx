...
import { Sparkles, Calendar, User, Target, Bell, ArrowRight, Zap } from 'lucide-react';
...
const Onboarding: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 max-w-lg mx-auto relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-blue-400/20 to-purple-400/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 animate-pulse"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-indigo-400/20 to-pink-400/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 animate-pulse" style={{ animationDelay: '1s' }}></div>
        ...
        {[1, 2, 3].map(i => (
          <div
            ...
            className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${
              i <= step ? 'bg-gradient-to-r from-blue-500 to-indigo-600' : 'bg-white/50'
            }`}
          />
        ))}
      </div>
      ...
      {step === 1 && (
        <div className={`flex-1 flex flex-col justify-center text-center relative z-10 transition-all duration-500 ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <p className="text-gray-600 mb-12 leading-relaxed text-lg max-w-md mx-auto">
            Welcome to your transformation. Over the next 80 days, we'll rebuild your habits, skills, and identity across 4 focused phases.
          </p>
          <div className="grid grid-cols-2 gap-4 mb-12 max-w-sm mx-auto">
            ...
            <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl shadow-lg border border-white">
              <div className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">80</div>
      ...
      {step === 2 && (
        <div className={`flex-1 flex flex-col pt-12 relative z-10 transition-all duration-500 ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <div className="flex items-center gap-3 mb-8">
            <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-2.5 rounded-xl shadow-lg">
              <User className="text-white" size={24} />
            </div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Who are you?</h2>
          </div>
          <label className="block mb-6">
            <span className="text-gray-700 font-semibold text-sm uppercase tracking-wide mb-3 block">Full Name</span>
          ...
          <div className="flex items-center gap-3 mb-6 mt-4">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Your Main Goal</h2>
          </div>
          <label className="block mb-8">
            <span className="text-gray-700 font-semibold text-sm uppercase tracking-wide mb-3 block">Why are you doing this?</span>
            <div className="relative">
              <textarea
      ...
      {step === 3 && (
        <div className={`flex-1 flex flex-col pt-12 relative z-10 transition-all duration-500 ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">When do we start?</h2>
          </div>
          <label className="block mb-8">
            <span className="text-gray-700 font-semibold text-sm uppercase tracking-wide mb-3 block">Start Date</span>
            <input
              type="date"
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Stay Tracked</h2>
          </div>
          <label className="flex items-center justify-between p-5 bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-gray-200 mb-8 cursor-pointer hover:border-indigo-400 transition-all shadow-lg group">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-7 rounded-full transition-all duration-300 relative ${formData.notificationsEnabled ? 'bg-gradient-to-r from-blue-500 to-indigo-600' : 'bg-gray-300'}`}>
                <div className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${formData.notificationsEnabled ? 'translate-x-5' : ''}`}></div>
              </div>
              <span className="font-semibold text-gray-800">Enable Daily Reminders</span>
            </div>
            <input
              type="checkbox"
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Your 80-day transformation begins on {new Date(formData.startDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}. Stay committed and watch yourself evolve.
                </p>
              </div>
            </div>
          </div>
